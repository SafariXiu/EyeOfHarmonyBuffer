...
public final class TerrainEngine {
    ...
    public static double sampleBaseHeight(
        ...
        // 群系偏移整合进 computeBaseHeightCore（baseT 线性偏移，取代旧的事后
        // smoothstep 重映射）。biomeScale 忽略：三层分解的 bandShape 已处理散布。
        double h1 = computeBaseHeightCore(worldX, worldZ, worldSeedInt, profile1, biomeBias);
        double h2 = computeBaseHeightCore(worldX, worldZ, worldSeedInt, profile2, biomeBias);

        // 宏包混合：权重直接线性混合，去掉 smoothstep(0.2,0.8) 的边界硬化，
        // 让宏包过渡带更宽、更平滑（无硬切）。
        double t;
        double sumW = w1 + w2;
        if (sumW > 0.0) {
        ...
        // 岩面细节已由 TerrainBaseHeight 的三层分解 detail 层承担（有界小振幅）。
        // 这里不再叠加 cliff 岩面噪声——旧的独立 cliff 层会放大高度带内的
        // 高频抖动（高原"乱窜"的根源之一）。
